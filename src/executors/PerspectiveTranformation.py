...
def select_best_corners(points, img_shape):
    ...
    try:
        ...
        # Allow for small floating point inaccuracies near zero
        epsilon = 1e-5
        if (np.all(signs >= -epsilon) and np.all(signs <= epsilon)) or (np.all(signs >= 0) or np.all(signs <= 0)):
...
class PerspectiveTransformation(Component):
    ...
    def remove_background_grabcut(self, img):
        ...
        try:
            ...
            return img # GrabCut başarısız olursa orijinal görüntüyü döndür
    ...
    def _correct_perspective_single_try(self, img,
        ...
        # Ön işleme
        preprocessed = img.copy() # Copy the input image to avoid modifying it directly
        ...
        if detection_method == 'hough':
            ...
            if edges is not None and np.sum(edges) > 0: # Check if edges were successfully created and are not all zero
                ...
                if lines is not None:
                    ...
                    lines = eliminate_duplicates(img, lines, threshold_distance) # Use original img for scale
                    if len(lines) >= 2: # Need at least 2 lines to find an intersection
                        cartesian = to_cartesian(img, lines) # Use original img for scale
                        intersections = get_intersections(img, cartesian) # Use original img for scale
            ...
            if edges_for_hough_p is not None and np.sum(edges_for_hough_p) > 0:
                ...
                if lines_p is not None and len(lines_p) > 0:
                     ...
                     intersections_p = get_intersections_from_segments(lines_p, img.shape) # Use original img shape
