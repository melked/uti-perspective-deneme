...
def detect_corners_harris(img_gray, blockSize=2, ksize=3, k=0.04, threshold=0.01):
    ...
    return np.float32(corners[:, ::-1]).reshape(-1, 2) if corners.size > 0 else np.array([], dtype=np.float32)
...
def get_intersections_from_segments(segments, img_shape):
    ...
    for x1, y1, x2, y2 in segments:
        # Uzun çizgiler oluşturmak için segmentleri uzat
        dx = x2 - x1
        dy = y2 - y1
        # Görüntü boyutunun 2 katı kadar uzat
    ...
    if extended_lines:
        ...
        for i, line1 in enumerate(extended_lines):
            ...
            for j, line2 in enumerate(extended_lines):
                ...
                x3, y3, x4, y4 = line2
                denom = (x1 - x2) * (y3 - y4) - (y1 - y2) * (x3 - x4)
                if denom == 0: # Paralel çizgiler
                    continue
                px = ((x1 * y2 - y1 * x2) * (x3 - x4) - (x1 - x2) * (x3 * y4 - y3 * x4)) / denom
                py = ((x1 * y2 - y1 * x2) * (y3 - y4) - (y1 - y2) * (x3 * y4 - y3 * x4)) / denom

                # Sadece görüntü içinde veya yakınındaki kesişimleri al
                if -width * 0.5 <= px <= width * 1.5 and -height * 0.5 <= py <= height * 1.5:
                    intersections.append([int(px), int(py)])

    return np.array(intersections, dtype=np.float32) if intersections else np.array([], dtype=np.float32)
...
class PerspectiveTransformation(Component):
    ...
    def preprocess_light_background(self, img):
        """Açık renk arka planlı belgeler için ön işleme"""
        # Kontrastı artır ve arka planı bastır
        gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
        clahe = cv2.createCLAHE(clipLimit=2.0, tileGridSize=(8, 8))
        gray_clahe = clahe.apply(gray)
        # Belgeyi belirginleştirmek için eşikleme
        _, thresh = cv2.threshold(gray_clahe, 0, 255, cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU)
        return cv2.cvtColor(thresh, cv2.COLOR_GRAY2BGR)


    def preprocess_dark_background(self, img):
        """Koyu renk arka planlı belgeler için ön işleme"""
        # Görüntüyü negatifine çevir ve kontrastı artır
    ...
    def preprocess_blurry(self, img):
        """Bulanık belgeler için ön işleme (Keskinleştirme)"""
        # Unsharp Mask veya diğer keskinleştirme yöntemleri kullanılabilir
    ...
    def remove_background_grabcut(self, img): # self parametresi eklendi
        ...
        try:
             ...
             # GrabCut başarısız olursa orijinal görüntüyü döndür
             return img
        return img
    ...
    def correct_perspective_enhanced(self, img,
                                      ...
                                      threshold_method='otsu'): # intermediate parametresi kaldırıldı, her zaman nihai çıktıyı döndürecek
        ...
        elif preprocess_method == 'light_background':
            preprocessed = self.preprocess_light_background(preprocessed)
        elif preprocess_method == 'dark_background':
            preprocessed = self.preprocess_dark_background(preprocessed)
        # 'default' durumda orijinal görüntü kullanılır, ek bir işlem yapılmaz

        if deblur:
            preprocessed = self.preprocess_blurry(preprocessed) # Bulanıklık giderme için yeni metod
            ...
            if use_adaptive_thresholding:
                ...
                if edge_detector == 'canny':
                     ...
                     if threshold_method == 'otsu':
                          ...
                          _, edges_for_hough_p = cv2.threshold(blurred, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_TRIANGLE)
                     else: # basic binary
                          _, edges_for_hough_p = cv2.threshold(blurred, 127, 255, cv2.THRESH_BINARY)
        ...
        # Çıktı boyutunu belirlerken oranı koru
        width_a = np.linalg.norm(src_quad[0] - src_quad[1])
        width_b = np.linalg.norm(src_quad[2] - src_quad[3])
        max_width = max(int(width_a), int(width_b))

        height_a = np.linalg.norm(src_quad[0] - src_quad[3])
        height_b = np.linalg.norm(src_quad[1] - src_quad[2])
        max_height = max(int(height_a), int(height_b))

        output_size = (max_width, max_height) # Bu, çıktı boyutunuz (out_w, out_h)
        destination = np.array([[0,0], [max_width-1,0], [max_width-1,max_height-1], [0,max_height-1]], dtype=np.float32)
    ...
    def _apply_perspective(self, src_img: np.ndarray):
        ...
        tries = [
            # Çeşitli genel senaryolar için temel denemeler
            ...
            {"preprocess_method": 'aggressive', "deblur": False, "remove_background": False, "remove_shadows_flag": True, "use_adaptive_thresholding": False, "edge_detector": 'canny', "blur_method": 'median', "median_blur_size": 31, "canny_threshold_max": 100, "canny_threshold_min": 30, "rho": 1, "theta": np.pi/180, "threshold_intersect": 100, "detection_method": 'hough'},

            # Farklı tespit yöntemleri
            {"preprocess_method": 'default', "remove_shadows_flag": False, "use_adaptive_thresholding": False, "blur_method": 'median', "median_blur_size": 51, "detection_method": 'contour', "contour_area_threshold_ratio": 0.05, "threshold_method": 'otsu'},
            {"preprocess_method": 'aggressive', "remove_shadows_flag": False, "use_adaptive_thresholding": False, "blur_method": 'median', "median_blur_size": 31, "detection_method": 'contour', "contour_area_threshold_ratio": 0.03, "threshold_method": 'otsu'},
            {"preprocess_method": 'default', "remove_shadows_flag": False, "use_adaptive_thresholding": False, "blur_method": 'median', "median_blur_size": 51, "detection_method": 'contour', "contour_area_threshold_ratio": 0.05, "threshold_method": 'triangle'},
            ...
            # Uyarlamalı eşikleme denemeleri
            {"preprocess_method": 'default', "deblur": False, "remove_background": False, "remove_shadows_flag": False, "use_adaptive_thresholding": True, "adaptive_block_size": 15, "adaptive_c_value": 5, "edge_detector": 'canny', "blur_method": 'median', "median_blur_size": 51, "rho": 1, "theta": np.pi/180, "threshold_intersect": 150, "detection_method": 'hough'},
             {"preprocess_method": 'aggressive', "deblur": False, "remove_background": False, "remove_shadows_flag": False, "use_adaptive_thresholding": True, "adaptive_block_size": 21, "adaptive_c_value": 10, "edge_detector": 'canny', "blur_method": 'median', "median_blur_size": 31, "rho": 1, "theta": np.pi/180, "threshold_intersect": 100, "detection_method": 'hough'},

            # Bulanık belgeler için keskinleştirme
            {"preprocess_method": 'default', "deblur": True, "remove_background": False, "remove_shadows_flag": False, "use_adaptive_thresholding": False, "edge_detector": 'canny', "blur_method": 'median', "median_blur_size": 51, "canny_threshold_max": 150, "canny_threshold_min": 50, "rho": 1, "theta": np.pi/180, "threshold_intersect": 150, "detection_method": 'hough'},

            # Koyu ve açık arka planlı belgeler için denemeler
            {"preprocess_method": 'dark_background', "deblur": False, "remove_background": False, "remove_shadows_flag": False, "use_adaptive_thresholding": False, "edge_detector": 'canny', "blur_method": 'median', "median_blur_size": 51, "canny_threshold_max": 150, "canny_threshold_min": 50, "rho": 1, "theta": np.pi/180, "threshold_intersect": 150, "detection_method": 'hough'},
            {"preprocess_method": 'light_background', "deblur": False, "remove_background": False, "remove_shadows_flag": False, "use_adaptive_thresholding": False, "edge_detector": 'canny', "blur_method": 'median', "median_blur_size": 51, "canny_threshold_max": 150, "canny_threshold_min": 50, "rho": 1, "theta": np.pi/180, "threshold_intersect": 150, "detection_method": 'hough'},
        ...
        for i, params in enumerate(tries):
            ...
            try:
                ...
                corrected_image, src_quad, output_size = self.correct_perspective_enhanced(
                    ...
                    threshold_distance=params.get("threshold_distance", 0.15),
                    perpendicular_margin=params.get("perpendicular_margin", np.pi/18),
                ...
                corrected_boxes = [] # Bu kısım hala boş, bounding box dönüşüm mantığı eklenmeli
